...
- 每道工序由 `route_step.work_image_slots` 独立选择 1～6 个动作/图片区，默认为 6；
- 视觉顺序依次为 `1`、`1,2`、`1,2,3`、`1,2 / 4,3`、`1,2,3 / 5,4`、`1,2,3 / 6,5,4`；
- 图片格数越少，图片与说明文字越大；IE 动作行数必须与格数一致；
- 图片默认空白，等待人工上传和确认；
...
-> 从路线步骤派生纵向流程图 PNG
-> 每个路线步骤建立一张横向指导书
-> 按每道工序的 work_image_slots 合并成 1～6 格图片与说明区
...
- 每页指导书格数必须等于该工序的 `work_image_slots`，并符合上述 1～6 格视觉顺序；
- 当作业步骤多于图片格数时，必须按原顺序合并说明，不得丢失文字；
- 每页包含 IE 工时记录表，动作行数等于图片格数，且没有伪造的现场实测值；
- 每页批准、审核、制作值栏为空；
- 无空白夹页、截断、重叠、乱码或表格越界；
- 实际渲染页数等于 `1 + 工序数`。
...
路线编辑模式的每道工序必须直接显示“几格”按钮，保存后将工序标记为 `needs_revision`并同步重建 DOCX/PDF/PNG。已批准路线不可直接修改；当目标格数小于已确认图片数时必须拒绝，不得隐藏或丢弃已确认图片。
