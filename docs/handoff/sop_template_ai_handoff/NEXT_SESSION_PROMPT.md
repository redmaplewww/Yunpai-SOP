...
要生成的是 yunpai.sop.hdmi-cable.multi-page.v4：第 1 页 A4 纵向完整流程图，第 2 页起按知识库路线逐工序生成 A4 横向标准作业指导书。它不是两页紧凑模板。每道工序由 route_step.work_image_slots 选择 1～6 格，IE 动作行数与格数一致。
...
验收：实际页数必须等于 1 + 路线步骤数；第 1 页纵向；其余页横向；没有空白夹页；每个工序有一张指导书；指导书格数、视觉顺序和 IE 行数与路线数据一致；图片待人工上传确认；已确认图片不得被静默截断；批准/审核/制作为空；逐页检查所有 PNG 后才可交付。
```
